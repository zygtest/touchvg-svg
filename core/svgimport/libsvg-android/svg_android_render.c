...
_svg_android_set_stroke_dash_array (void *closure, double *dash, int num_dashes)
{
	...
	if (svg_android->state->num_dashes) {
		...
		int effect = ANDROID_GET_DASHEFFECT(
			svg_android, farr,
			...
			double x0, double y0)
{
	...
	int new_matrix;
	int old_matrix = svg_android->state->matrix;
			 ...
			 int offscreen_bitmap)
{
	...
	else
	{
		...
		if (offscreen_bitmap)
		{
			int new_canvas = ANDROID_CANVAS_CREATE(svg_android, offscreen_bitmap);
		DEBUG_ANDROID("P S 5");
